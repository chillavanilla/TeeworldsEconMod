...
class KillsController:
    ...
    def handle_kills_06(self, data):
        ...
        killer = self.players_controller.get_player_by_name(killer_name)

        victim_start = data.find(":", killer_end) + 1
        victim_end = data.find("' weapon=", victim_start + 1)
        victim_name = data[victim_start:victim_end]
        victim = self.players_controller.get_player_by_name(victim_name)
    ...
    def handle_kills(self, timestamp, data):
        ...
        self.players_controller.check_flagger_kill(kill.victim.name, kill.killer.name)
